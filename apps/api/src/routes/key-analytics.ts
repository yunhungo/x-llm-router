      ...
      pool.query(
        `WITH bounds AS (
         ...
         SELECT GREATEST(b.bucket, $2::timestamptz) AS bucket,
                ...
                COALESCE(sum((s.cost_breakdown->>'inputCostUsd')::numeric), 0)::float8 AS "inputCostUsd",
                COALESCE(sum((s.cost_breakdown->>'cachedInputCostUsd')::numeric), 0)::float8 AS "cachedInputCostUsd",
                COALESCE(sum((s.cost_breakdown->>'outputCostUsd')::numeric), 0)::float8 AS "outputCostUsd",
