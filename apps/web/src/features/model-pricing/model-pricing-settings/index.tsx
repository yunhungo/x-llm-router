...
import { Button, ComboboxInput, Field, Input, Modal, Skeleton } from '../../../components/ui';
import type { Provider } from '../../../types';
import { deleteModelPrice, loadModelPrices, upsertModelPrice } from '../model-pricing.api';
import {
  emptyPriceDraft,
  parsePriceValues,
  priceDraft,
...
import './model-pricing-settings.css';
...
export function ModelPricingSettings({
  ...
  const reloadPrices = useCallback(async () => {
    const response = await loadModelPrices(keyId);
  ...
  return (
    <section className="panel flush-panel model-pricing-panel key-settings-model-pricing">
      <div className="panel-heading model-pricing-heading">
        <div>
          <h2>模型价格</h2>
          <span className="panel-note">当前 API Key · 仅展示已配置记录 · USD / 1M tokens</span>
        </div>
        <div>
          <Button
            onClick={() => {
      ...
      {loading ? (
        <div className="model-pricing-loading">
          <Skeleton height={240} />
        </div>
      ) : (
